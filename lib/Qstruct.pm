...
B<Qstruct> is a binary data serialisation format. Unlike L<Storable>, L<Data::MessagePack>, L<Sereal>, L<CBOR::XS> etc, Qstruct requires a schema. This makes it more like the various encodings of L<ASN.1|http://www.itu.int/en/ITU-T/asn1/Pages/introduction.aspx> or L<Protocol Buffers|https://developers.google.com/protocol-buffers/>.

In addition to the above, Qstruct is heavily inspired by L<Cap'n Proto|http://kentonv.github.io/capnproto/> and I am indebted to Kenton Varda for thinking through and publishing many of the details of how this type of serialisation should work. Qstructs originally came about as an attempt to implement Cap'n Proto in perl.
...
C<Portable>: All integers and floating point numbers are stored in little-endian byte order and can start at unaligned offsets (if you load a message from an unaligned offset). Despite these restrictions, Qstructs can be used on any CPU, even ones that are big-endian or have strict alignment requirements.
