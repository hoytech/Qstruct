...
Only numeric (ie integer or floating point) types values may be used in fixed arrays: Strings, blobs, bools, and qstructs cannot be stored in fixed arrays.
...
Fixed arrays are stored inline in the body of the message. Their size and offset from the start of the message is always known exactly so there is no need to store/compute offset pointers or lengths. However, you can never change the size of the array so they should only be used when you are 100% certain that you will never want to expand, shrink, or remove this field.
...
The qstruct definitions can be shuffled around in any order in the schema.

You can add new fields to a qstruct as long as you don't change existing fields' types or C<@> ids and the C<@> ids are always repeating without gaps. Any messages that were created with the old schema will still be loadable: Accessing new fields in old messages will return default values.
